...
int allocate_frame(pgtbl_entry_t *p) {
	...
	// Get Victim from pagetable
	pgtbl_entry_t *vic = coremap[frame].pte;

	// Victim is Dirty
	if (vic->frame & PG_DIRTY) {

		// Swap out Victim page
		int swap_offset = swap_pageout(frame, vic->swap_off);

		// Check if swap was successful
		if(swap_offset == INVALID_SWAP){
			perror("Invalid Swap out");
			exit(1);
		}

		// Set page offset to swap offset
		vic->swap_off = swap_offset;

		// Set onswap bit and clear dirty bit
		vic->frame |= PG_ONSWAP;
		vic->frame &= ~PG_DIRTY;
		...
		// Increment clean counter since page is not dirty
		evict_clean_count++;
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// If pde is not set, then initialize second level tables.
	if(!(pgdir[idx].pde & PG_VALID)){
		pgdir[idx] = init_second_level();
	}

	// Pointer to second level table
	pgtbl_entry_t *second = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);

	// Initialize p to page from second table at index vaddr
	p = second + PGTBL_INDEX(vaddr);

	// Check if p is Valid
	if (p->frame & PG_VALID){

		// Increment hit_count because there is a match
		hit_count ++;

	} else { //No match

		// Allocate space in physical frame
		int frame = allocate_frame(p);

		// Check if frame was allocated correctly
		if(frame == -1){
			perror("Allocate Frame Error");
			exit(1);
		}

		// First time referencing so initialize frame
		if(!(p->frame & PG_ONSWAP)){
			...
			// Fill in with information from swap
		} else {
